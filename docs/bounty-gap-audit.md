...
- Production is live. GitHub Actions deploy-from-main was proven by run `25216210247`; the latest R2-enabled production deploy was completed locally from `main` after R2 was enabled and bucket `annotated-canvas-media` was created.
...
- Real OAuth provider exchange and owned-media 240p policy remain open. Recorded audio now has a production R2 storage/fetch smoke, but owned-video 240p/sub-480p processing is still unresolved.
...
- **Extension p95 proof**: PR #43 records selected-text, media-time, current-tab publish, over-90 no-network rejection, stored API reads, public permalinks, and pre-R2 audio fallback.
- **R2 audio proof**: #26 records `storage=r2`, `status=stored`, byte fetch, and a published audio annotation permalink after R2 enablement.
- **Reviewer journey proof**: #38 records the marketing/feed/signup/local-extension/capture/return loop plus Krug guardrails before demo filming; the home route now includes real extension smoke screenshots for cold traffic.
- **Human/platform blockers**: #24 needs real Google/X provider credentials and token exchange; #26 still needs a 240p/sub-480p owned-media decision.
...
| Add commentary and annotations | Working now for text and R2-backed audio | Text commentary publishes; approved public annotation exists; R2 audio upload/fetch and audio annotation publish smoke exist. | Audio playback UI should be polished in the final demo; owned-video remains separate. | #26 |
| Landing page linking back to original source | Working now | Approved public permalink `ann_d586ad40-058a-42c1-b6d7-8e0e691cfae4` returns the canonical Pages `permalink_url` and source link. | Continue to verify every created annotation carries `source_url`. | #21, closed #28 |
| Public social feed | Working now | Public feed includes the approved smoke annotation; profile route returns `200`. | Final packet should use fresh proof immediately before external submission. | closed #28 |
| Follow and engage with annotations | Working now | Comments and engagement paths are implemented; follow UI was wired to API before PR #31 merge. | Include follow/comment proof in final demo, not only local tests. | closed #28, closed #25 |
...
| URL input or current page | Working now | Web URL composer is tested; extension current-page capture has production p95 proof. | Final demo should show both paths clearly. | closed #28, closed #23, closed #30 |
| Prompt for start/end or text section | Working now | UI/API support text quote and media start/end; extension p95 proves selected quote and exact media seconds. | Final demo should reuse the p95 evidence links. | closed #28, closed #23, closed #30 |
...
| Text or recorded audio commentary | Working now at API/storage level | Text works; production audio upload returns `storage=r2`, `status=stored`; API rejects arbitrary or unfinalized `audio_asset_id`; GET returns stored bytes; audio annotation publish smoke exists. | Web/extension playback polish remains follow-up; owned-video remains separate. | #26 |
| Users can leave comments | Working now | Public smoke created `cmt_73c82db2-d4db-4661-b92e-84b12b4e74e7`. | Re-run before final submission if the seed data changes. | closed #25, closed #28 |
...
| All content links to original source | Working now | Third-party clip contracts require `source_url`; permalink shows original source link; extension p95 stores original source metadata. | Keep this invariant in final demo. | #21, closed #28 |
| Submit to annotated.lovable.app | Deployed but limited | Reviewer packet has live URLs, demo script, approved smoke annotation, p95 extension evidence, and honest blocker list. | Human decision to submit with #24/#26 disclosed or wait for those gates. | closed #28 |
...
- [ ] Close only after #24/#26 are completed or deliberately disclosed by the submitter.
...
- [x] Recorded audio has durable R2 storage.
- [x] Upload/finalize metadata prevents arbitrary asset IDs from being published as audio commentary.
- [x] Production API can load recorded audio commentary bytes by asset id.
...
- [x] Oversized audio and unsupported audio content types are rejected before storage.
...
- **p50 test**: text commentary, missing-audio-asset rejection, and R2 audio upload/fetch behavior are clear.
- **p95 test**: oversized/unsupported audio is rejected, finalized audio persists, and owned-video rendition policy is enforceable or explicitly excluded.

### Closed #28 Final Bounty Submission Package

- [x] Public URLs, demo routes, extension install steps, and smoke IDs are current.
- [x] Demo script covers sign-in state, URL/current-page capture, text/media commentary, feed, follow/comment, claim filing, and source links.
- [x] Known limitations are copied into the external submission packet.
- [x] Human submitter can decide whether to post with disclosed gaps or wait.
...
- **#26**: R2/audio is working at API/storage level; owned-media 240p policy is still undecided.
- External submission should disclose #24/#26 or wait for those gates.
